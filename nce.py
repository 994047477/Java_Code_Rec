...
class NCELoss(nn.Module):
    ...
    def get_scores(self, idx, scores):
        ...
        #the K = self.K + 1
        K = idx.size(1)
        idx_increment = N * torch.arange(B).view(B,1).long().cuda() *torch.ones(K).long().cuda()
    ...
    def get_noise(self, batch_size, uniform = False):
        ...
        if uniform:
            ...
            noise = np.random.choice(self.N, self.K * batch_size, replace= True, p = self.Q.cpu().data )
        noise_idx = Variable(torch.LongTensor(noise).view(batch_size, self.K))
        return noise_idx

    def get_combined_idx(self, target_idx, noise_idx):
        return torch.cat((target_idx.view(-1,1), noise_idx.cuda()), 1)
